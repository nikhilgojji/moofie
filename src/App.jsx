              ...
              <section className="home-course-content">
                ...
                {!resources && loadingCourseId === selectedCourse.id ? (
                  ...
                  <div className="home-resource-grid" data-canvas-restricted={["restricted", "unavailable"].includes(sectionStatus) || undefined}>
                    {sectionStatus && sectionStatus !== "current" && <div className="course-sync-notice" role="status"><p>{sectionStatus === "restricted" ? "Canvas does not currently allow your account to read this section." : sectionStatus === "unavailable" ? "Canvas did not return this section. Its availability may have changed." : sectionStatus === "mismatch" ? "Some content could not be verified. Showing available Canvas content while Moofie checks again." : "This section could not refresh. Previously loaded content may be out of date. Moofie will retry automatically."}</p><button type="button" onClick={() => window.dispatchEvent(new Event("moofie:refresh"))}>Check Canvas now</button></div>}
